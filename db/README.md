...
Operational gravesite status uses `gravesite_status_types.id` as the canonical UUID lookup key through `gravesites.status_type_id`, but the displayed status is derived from record facts. `needs_review` is an explicit human flag stored on the gravesite. Otherwise, an active burial makes the gravesite `occupied`; `reserved` is used when someone has claimed the gravesite before a deed exists; no active burial plus a current deed or ownership right for the gravesite or its lot makes it `sold`; and no active burial plus no current deed or ownership right makes it `available`. Remaining unmatched cases display as `unknown`. Application reads, searches, and map rendering use this derived status; the legacy `gravesites.status` lowercase code column has been removed.
