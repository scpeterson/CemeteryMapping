...
Operational gravesite status uses `gravesite_status_types.id` as the canonical UUID lookup key through `gravesites.status_type_id`. Application reads, writes, searches, map rendering, and import scripts should use that lookup relationship. The legacy `gravesites.status` lowercase code is retained only as a temporary compatibility fallback and is synchronized by database trigger until a later migration removes it. `needs_review` is distinct from `unknown`: use `needs_review` when a known record needs staff review, and `unknown` when the status has not been determined.
