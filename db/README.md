...
Operational gravesite status uses `gravesite_status_types.id` as the canonical UUID lookup key through `gravesites.status_type_id`, but the displayed status is derived from record facts. `reserved` and `needs_review` are explicit human flags stored on the gravesite. Otherwise, an active burial makes the gravesite `occupied`; no active burial and no current deed or ownership right for the gravesite or its lot makes it `available`; deeded-but-unburied and other unmatched cases display as `unknown`. `sold` is deprecated and inactive because ownership/deed data is now modeled separately from operational status. Application reads, searches, and map rendering use this derived status; the legacy `gravesites.status` lowercase code column has been removed.
