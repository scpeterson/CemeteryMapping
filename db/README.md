...
Operational gravesite status is stored as a lowercase lookup code in `gravesites.status` and enforced by `gravesite_status_types`. `needs_review` is distinct from `unknown`: use `needs_review` when a known record needs staff review, and `unknown` when the status has not been determined.
