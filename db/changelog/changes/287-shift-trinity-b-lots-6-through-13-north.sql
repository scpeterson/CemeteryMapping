...
--rollback UPDATE lots SET geometry = ST_Translate(lots.geometry, 0, ST_Y(ST_Project(ST_Centroid(ST_Collect(lots.geometry)) OVER ()::geography, 0.762, radians(180))::geometry) - ST_Y(ST_Centroid(ST_Collect(lots.geometry)) OVER ()))::geometry(MultiPolygon, 4326), updated_at = now() WHERE lots.facility_id = '1' AND lots.section_id = 'B' AND lots.lot_id IN ('6', '7', '8', '11', '12', '13') AND lots.block_id IS NULL AND lots.deleted_at IS NULL;
