...
--changeset cemeterymapping:118-split-c-0186-fowler-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0186'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id IN ('TLC-GPS-0186', 'TLC-GPS-0186-02')
    AND upper(COALESCE(gravesites.section_id, '')) = 'C'
  ORDER BY gravesites.gravesite_id = 'TLC-GPS-0186' DESC
  LIMIT 1
),
...
chester_gravesite AS (
  UPDATE gravesites
  SET
    name = 'Chester J Fowler',
    grave_id = '0186B',
    gravesite_id = 'TLC-GPS-0186-02',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0186 using fixed marker TLC-HS-0186 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'Chester J Fowler assigned to the southern gravesite when splitting shared Fowler marker on 2026-06-19.'
    ),
    updated_at = now()
...
helen_gravesite AS (
  ...
  SELECT
    ...
    status_type_id,
    'operational',
    'Split from TLC-GPS-0186 using fixed marker TLC-HS-0186 as shared north/south boundary.',
    'estimated',
    'Helen E Fowler assigned to the northern gravesite when splitting shared Fowler marker on 2026-06-19.',
    now()
  FROM chester_gravesite
...
marker_context AS (
  SELECT
    ...
    chester_gravesite.headstone_uuid
  FROM chester_gravesite
...
updated_chester_burials AS (
  ...
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'chester j fowler'
...
updated_helen_burials AS (
  ...
  SET
    ...
    gravesite_id = 'TLC-GPS-0186-01',
    updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'helen e fowler'
  RETURNING burials.id
),
