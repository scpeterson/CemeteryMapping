...
--changeset cemeterymapping:396-split-a-0066-steele-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0066' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0066' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0066 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'wilbert b steele'
      AND headstones.headstone_id = 'TLC-HS-0066'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'anna s steele'
      AND headstones.headstone_id = 'TLC-HS-0066'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Wilbert B Steele burial and one linked active Anna S Steele burial must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR NOT EXISTS (
    ...
    WHERE target.cemetery_id = (SELECT cemetery_id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0066' AND deleted_at IS NULL)
      AND (target.gravesite_id = 'TLC-GPS-0066-01' OR (target.section_id = 'A' AND target.grave_id = '0066A'))
  ),
  'new gravesite TLC-GPS-0066-01 / A-0066A must be unused'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR EXISTS (
    ...
    WHERE g.gravesite_id = 'TLC-GPS-0066' AND g.section_id = 'A' AND g.deleted_at IS NULL
      AND h.headstone_id = 'TLC-HS-0066' AND h.deleted_at IS NULL
      AND (SELECT count(*) FROM burials b JOIN headstone_burials hb ON hb.burial_uuid = b.id
        WHERE hb.headstone_uuid = h.id AND hb.deleted_at IS NULL AND b.deleted_at IS NULL
          AND b.gravesite_uuid = g.id AND b.full_name IN ('Wilbert B Steele', 'Anna S Steele')) = 2
  ),
  'both Steele burials and the marker must belong to active Section A gravesite TLC-GPS-0066'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0066' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0066'
...
wilbert_gravesite AS (
  UPDATE gravesites
  SET name = 'Wilbert B Steele', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0066 using fixed marker TLC-HS-0066 as the boundary between A-0066 and A-0066A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Wilbert B Steele retained in original gravesite A-0066 and moved south; Anna S Steele was assigned the new grave north of the fixed shared marker on 2026-09-16. Small estimated overlaps with neighboring gravesites require field review; see ADR 0059.'
    ), updated_at = now()
...
anna_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Anna S Steele', facility_id,
    section_id, block_id, lot_id, '0066A', 'TLC-GPS-0066-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0066 using fixed marker TLC-HS-0066 as the boundary between A-0066 and A-0066A.',
    'estimated', 'Anna S Steele assigned to new gravesite A-0066A immediately north of A-0066 on 2026-09-16. Small estimated overlaps with neighboring gravesites require field review; see ADR 0059.', now()
...
marker_context AS (
  SELECT wilbert_gravesite.id AS wilbert_gravesite_uuid,
    anna_gravesite.id AS anna_gravesite_uuid, wilbert_gravesite.headstone_uuid
  FROM wilbert_gravesite CROSS JOIN anna_gravesite
),
updated_wilbert_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.wilbert_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0066', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'wilbert b steele'
    AND EXISTS (
...
updated_anna_burial AS (
  ...
  SET gravesite_uuid = marker_context.anna_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0066-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'anna s steele'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, wilbert_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_wilbert_burial.id
  FROM marker_context CROSS JOIN updated_wilbert_burial
  UNION ALL
...
UPDATE headstones
SET gravesite_uuid = marker_context.wilbert_gravesite_uuid, updated_at = now()
FROM marker_context
