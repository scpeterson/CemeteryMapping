...
--changeset cemeterymapping:358-split-c-0402-pagano-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0402' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0402' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0402 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'constance "connie" pagano'
      AND headstones.headstone_id = 'TLC-HS-0402'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'pat pagano'
      AND headstones.headstone_id = 'TLC-HS-0402'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Constance "Connie" Pagano burial and one linked active Pat Pagano burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0402' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0402'
...
constance_gravesite AS (
  UPDATE gravesites
  SET name = 'Constance "Connie" Pagano', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0402 using fixed marker TLC-HS-0402 as the boundary between C-0402 and C-0402A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Constance "Connie" Pagano retained in original gravesite C-0402 and moved south; Pat Pagano was assigned the new grave north of the fixed shared marker on 2026-09-01.'
    ), updated_at = now()
...
pat_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Pat Pagano', facility_id,
    section_id, block_id, lot_id, '0402A', 'TLC-GPS-0402-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0402 using fixed marker TLC-HS-0402 as the boundary between C-0402 and C-0402A.',
    'estimated', 'Pat Pagano assigned to new gravesite C-0402A immediately north of C-0402 on 2026-09-01.', now()
  FROM constance_gravesite
...
marker_context AS (
  SELECT constance_gravesite.id AS constance_gravesite_uuid,
    pat_gravesite.id AS pat_gravesite_uuid, constance_gravesite.headstone_uuid
  FROM constance_gravesite CROSS JOIN pat_gravesite
),
updated_constance_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.constance_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0402', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'constance "connie" pagano'
    AND EXISTS (
...
updated_pat_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.pat_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0402-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'pat pagano'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, constance_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
  SELECT headstone_uuid, pat_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_constance_burial.id
  FROM marker_context CROSS JOIN updated_constance_burial
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_pat_burial.id
  FROM marker_context CROSS JOIN updated_pat_burial
  ON CONFLICT (headstone_uuid, burial_uuid) DO UPDATE SET
    deleted_at = NULL, deleted_by = NULL, delete_reason = NULL
)
UPDATE headstones
SET gravesite_uuid = marker_context.constance_gravesite_uuid, updated_at = now()
FROM marker_context
