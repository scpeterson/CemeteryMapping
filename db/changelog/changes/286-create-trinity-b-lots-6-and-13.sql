...
--changeset cemeterymapping:286-create-trinity-b-lots-6-and-13 splitStatements:false
WITH source_lots AS (
  SELECT
    lots.*,
    CASE lots.lot_id
...
--rollback DELETE FROM lots WHERE facility_id = '1' AND section_id = 'B' AND lot_id IN ('6', '13') AND block_id IS NULL AND NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesites.lot_uuid = lots.id);
