...
--changeset cemeterymapping:266-split-c-0288-soergel-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0288'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0288'
...
peter_gravesite AS (
  ...
  SET
    ...
    geometry_source = 'Split from TLC-GPS-0288 using fixed marker TLC-HS-0288 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'Peter Soergel retained in original gravesite C-0288 and moved south when splitting the shared Soergel marker on 2026-07-27.'
    ),
    updated_at = now()
...
margaret_gravesite AS (
  ...
  SELECT
    ...
    status_type_id,
    'operational',
    'Split from TLC-GPS-0288 using fixed marker TLC-HS-0288 as shared north/south boundary.',
    'estimated',
    'Margaret Soergel assigned to new northern gravesite C-0288A when splitting the shared Soergel marker on 2026-07-27.',
    now()
...
soergel_burials AS (
  ...
  WHERE burials.deleted_at IS NULL
    ...
    AND lower(split_part(trim(COALESCE(burials.first_name, '')), ' ', 1)) IN ('peter', 'margaret')
),
updated_peter_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.peter_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0288',
    updated_at = now()
  FROM marker_context, soergel_burials
  WHERE burials.id = soergel_burials.id
    AND soergel_burials.normalized_given_name = 'peter'
...
updated_margaret_burials AS (
  ...
  SET
    ...
    gravesite_id = 'TLC-GPS-0288-01',
    updated_at = now()
  FROM marker_context, soergel_burials
  WHERE burials.id = soergel_burials.id
    AND soergel_burials.normalized_given_name = 'margaret'
  RETURNING burials.id
),
