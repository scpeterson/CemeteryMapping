...
--changeset cemeterymapping:353-split-c-0395-wochley-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0395' AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM headstones
    WHERE headstone_id = 'TLC-HS-0395' AND geometry IS NOT NULL AND deleted_at IS NULL
  ),
  'active marker TLC-HS-0395 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'arthur dennis wochley'
      AND headstones.headstone_id = 'TLC-HS-0395'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'luceil wochley'
      AND headstones.headstone_id = 'TLC-HS-0395'
      AND burials.deleted_at IS NULL
      AND headstone_burials.deleted_at IS NULL
      AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Arthur Dennis Wochley burial and one linked active Luceil Wochley burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0395'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0395'
...
arthur_gravesite AS (
  UPDATE gravesites
  SET
    name = 'Arthur Dennis Wochley',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0395 using fixed marker TLC-HS-0395 as the boundary between C-0395 and C-0395A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Arthur Dennis Wochley retained in original gravesite C-0395 and moved south; Luceil Wochley was assigned the new grave north of the fixed shared marker on 2026-08-31.'
    ),
    updated_at = now()
...
luceil_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Luceil Wochley', facility_id,
    section_id, block_id, lot_id, '0395A', 'TLC-GPS-0395-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0395 using fixed marker TLC-HS-0395 as the boundary between C-0395 and C-0395A.',
    'estimated',
    'Luceil Wochley assigned to new gravesite C-0395A immediately north of C-0395 on 2026-08-31.',
    now()
...
marker_context AS (
  SELECT
    ...
    luceil_gravesite.id AS luceil_gravesite_uuid,
    arthur_gravesite.headstone_uuid
  FROM arthur_gravesite CROSS JOIN luceil_gravesite
),
updated_arthur_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.arthur_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0395', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'arthur dennis wochley'
    AND EXISTS (
...
updated_luceil_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.luceil_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0395-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'luceil wochley'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, arthur_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
  SELECT headstone_uuid, luceil_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  FROM marker_context CROSS JOIN updated_arthur_burial
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_luceil_burial.id
  FROM marker_context CROSS JOIN updated_luceil_burial
  ON CONFLICT (headstone_uuid, burial_uuid) DO UPDATE SET
    deleted_at = NULL, deleted_by = NULL, delete_reason = NULL
)
UPDATE headstones
SET gravesite_uuid = marker_context.arthur_gravesite_uuid, updated_at = now()
FROM marker_context
