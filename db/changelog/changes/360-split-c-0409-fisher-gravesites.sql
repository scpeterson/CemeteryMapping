...
--changeset cemeterymapping:360-split-c-0409-fisher-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0409' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0409' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0409 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'james t fisher'
      AND headstones.headstone_id = 'TLC-HS-0409'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'arlie d fisher'
      AND headstones.headstone_id = 'TLC-HS-0409'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active James T Fisher burial and one linked active Arlie D Fisher burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0409' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0409'
...
james_gravesite AS (
  UPDATE gravesites
  SET name = 'James T Fisher', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0409 using fixed marker TLC-HS-0409 as the boundary between C-0409 and C-0409A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'James T Fisher retained in original gravesite C-0409 and moved south; Arlie D Fisher was assigned the new grave north of the fixed shared marker on 2026-09-01.'
    ), updated_at = now()
...
arlie_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Arlie D Fisher', facility_id,
    section_id, block_id, lot_id, '0409A', 'TLC-GPS-0409-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0409 using fixed marker TLC-HS-0409 as the boundary between C-0409 and C-0409A.',
    'estimated', 'Arlie D Fisher assigned to new gravesite C-0409A immediately north of C-0409 on 2026-09-01.', now()
...
marker_context AS (
  SELECT james_gravesite.id AS james_gravesite_uuid,
    arlie_gravesite.id AS arlie_gravesite_uuid, james_gravesite.headstone_uuid
  FROM james_gravesite CROSS JOIN arlie_gravesite
),
updated_james_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.james_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0409', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'james t fisher'
    AND EXISTS (
...
updated_arlie_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.arlie_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0409-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'arlie d fisher'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  UNION ALL
  SELECT headstone_uuid, arlie_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_arlie_burial.id
  FROM marker_context CROSS JOIN updated_arlie_burial
