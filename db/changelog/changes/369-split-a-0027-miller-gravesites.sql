...
--changeset cemeterymapping:369-split-a-0027-miller-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0027' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0027' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0027 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'charles "cart" miller'
      AND headstones.headstone_id = 'TLC-HS-0027'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'martha b miller'
      AND headstones.headstone_id = 'TLC-HS-0027'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Charles "Cart" Miller burial and one linked active Martha B Miller burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0027' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0027'
...
charles_gravesite AS (
  UPDATE gravesites
  SET name = 'Charles "Cart" Miller', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0027 using fixed marker TLC-HS-0027 as the boundary between A-0027 and A-0027A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Charles "Cart" Miller retained in original gravesite A-0027 and moved south; Martha B Miller was assigned the new grave north of the fixed shared marker on 2026-09-02.'
    ), updated_at = now()
...
martha_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Martha B Miller', facility_id,
    section_id, block_id, lot_id, '0027A', 'TLC-GPS-0027-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0027 using fixed marker TLC-HS-0027 as the boundary between A-0027 and A-0027A.',
    'estimated', 'Martha B Miller assigned to new gravesite A-0027A immediately north of A-0027 on 2026-09-02.', now()
...
marker_context AS (
  SELECT charles_gravesite.id AS charles_gravesite_uuid,
    martha_gravesite.id AS martha_gravesite_uuid, charles_gravesite.headstone_uuid
  FROM charles_gravesite CROSS JOIN martha_gravesite
),
updated_charles_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.charles_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0027', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'charles "cart" miller'
    AND EXISTS (
...
updated_martha_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.martha_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0027-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'martha b miller'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  UNION ALL
  SELECT headstone_uuid, martha_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_martha_burial.id
  FROM marker_context CROSS JOIN updated_martha_burial
