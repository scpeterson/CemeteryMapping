...
--changeset cemeterymapping:334-split-c-0427-vandevort-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id IN ('TLC-GPS-0426', 'TLC-GPS-0428') AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id = 'TLC-GPS-0427'
      AND upper(COALESCE(section_id, '')) = 'C'
      AND deleted_at IS NULL
  ),
  'active Section C gravesite TLC-GPS-0427 must exist when its neighboring operational gravesites exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0427' AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM headstones
    WHERE headstone_id = 'TLC-HS-0427' AND geometry IS NOT NULL AND deleted_at IS NULL
  ),
  'active marker TLC-HS-0427 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(full_name, '')) = 'david lynn vandevort' AND deleted_at IS NULL
  ) = 1
  AND (
    SELECT count(*) FROM burials
    WHERE lower(COALESCE(full_name, '')) = 'mildred abbott vandevort' AND deleted_at IS NULL
  ) = 1,
  'exactly one active David Lynn Vandevort burial and one active Mildred Abbott Vandevort burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0427'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0427'
...
david_gravesite AS (
  UPDATE gravesites
  SET
    name = 'David Lynn Vandevort',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0427 using fixed marker TLC-HS-0427 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'David Lynn Vandevort retained in original gravesite C-0427 and moved south when splitting the shared Vandevort marker on 2026-08-25.'
    ),
    updated_at = now()
...
mildred_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Mildred Abbott Vandevort', facility_id,
    section_id, block_id, lot_id, '0427A', 'TLC-GPS-0427-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0427 using fixed marker TLC-HS-0427 as shared north/south boundary.',
    'estimated',
    'Mildred Abbott Vandevort assigned to new northern gravesite C-0427A when splitting the shared Vandevort marker on 2026-08-25.',
    now()
...
marker_context AS (
  SELECT
    ...
    mildred_gravesite.id AS mildred_gravesite_uuid,
    david_gravesite.headstone_uuid
  FROM david_gravesite CROSS JOIN mildred_gravesite
),
updated_david_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.david_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0427', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'david lynn vandevort'
    AND EXISTS (
...
updated_mildred_burial AS (
  ...
  SET gravesite_uuid = marker_context.mildred_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0427-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'mildred abbott vandevort'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, david_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_david_burial.id
  FROM marker_context CROSS JOIN updated_david_burial
  UNION ALL
...
UPDATE headstones
SET gravesite_uuid = marker_context.david_gravesite_uuid, updated_at = now()
FROM marker_context
