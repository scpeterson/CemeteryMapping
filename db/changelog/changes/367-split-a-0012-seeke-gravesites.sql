...
--changeset cemeterymapping:367-split-a-0012-seeke-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0012' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0012' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0012 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'frekerick seeke'
      AND headstones.headstone_id = 'TLC-HS-0012'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'marie seeke'
      AND headstones.headstone_id = 'TLC-HS-0012'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Frederick Seeke burial and one linked active Marie Seeke burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0012' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0012'
...
frederick_gravesite AS (
  UPDATE gravesites
  SET name = 'Frederick Seeke', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0012 using fixed marker TLC-HS-0012 as the boundary between A-0012 and A-0012A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Frederick Seeke retained in original gravesite A-0012 and moved south; Marie Seeke was assigned the new grave north of the fixed shared marker on 2026-09-02.'
    ), updated_at = now()
...
marie_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Marie Seeke', facility_id,
    section_id, block_id, lot_id, '0012A', 'TLC-GPS-0012-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0012 using fixed marker TLC-HS-0012 as the boundary between A-0012 and A-0012A.',
    'estimated', 'Marie Seeke assigned to new gravesite A-0012A immediately north of A-0012 on 2026-09-02.', now()
  FROM frederick_gravesite
...
marker_context AS (
  SELECT frederick_gravesite.id AS frederick_gravesite_uuid,
    marie_gravesite.id AS marie_gravesite_uuid, frederick_gravesite.headstone_uuid
  FROM frederick_gravesite CROSS JOIN marie_gravesite
),
updated_frederick_burial AS (
  UPDATE burials
  SET first_name = 'Frederick', full_name = 'Frederick Seeke',
    gravesite_uuid = marker_context.frederick_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0012', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'frekerick seeke'
    AND EXISTS (
...
updated_marie_burial AS (
  ...
  SET gravesite_uuid = marker_context.marie_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0012-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'marie seeke'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, frederick_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_frederick_burial.id
  FROM marker_context CROSS JOIN updated_frederick_burial
  UNION ALL
...
UPDATE headstones
SET gravesite_uuid = marker_context.frederick_gravesite_uuid, updated_at = now()
FROM marker_context
