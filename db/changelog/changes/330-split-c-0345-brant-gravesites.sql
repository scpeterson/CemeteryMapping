...
--changeset cemeterymapping:330-split-c-0345-brant-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id IN ('TLC-GPS-0344', 'TLC-GPS-0346') AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id = 'TLC-GPS-0345'
      AND upper(COALESCE(section_id, '')) = 'C'
      AND deleted_at IS NULL
  ),
  'active Section C gravesite TLC-GPS-0345 must exist when its neighboring operational gravesites exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0345' AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM headstones
    WHERE headstone_id = 'TLC-HS-0345' AND geometry IS NOT NULL AND deleted_at IS NULL
  ),
  'active marker TLC-HS-0345 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(full_name, '')) = 'herbert george brant' AND deleted_at IS NULL
  ) = 1
  AND (
    SELECT count(*) FROM burials
    WHERE lower(COALESCE(full_name, '')) = 'helen s brant' AND deleted_at IS NULL
  ) = 1,
  'exactly one active Herbert George Brant burial and one active Helen S Brant burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0345'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0345'
...
herbert_gravesite AS (
  UPDATE gravesites
  SET
    name = 'Herbert George Brant',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0345 using fixed marker TLC-HS-0345 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Herbert George Brant retained in original gravesite C-0345 and moved south when splitting the shared Brant marker on 2026-08-24.'
    ),
    updated_at = now()
...
helen_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Helen S Brant', facility_id,
    section_id, block_id, lot_id, '0345A', 'TLC-GPS-0345-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0345 using fixed marker TLC-HS-0345 as shared north/south boundary.',
    'estimated',
    'Helen S Brant assigned to the new northern gravesite C-0345A when splitting the shared Brant marker on 2026-08-24.',
    now()
  FROM herbert_gravesite
...
marker_context AS (
  SELECT
    herbert_gravesite.id AS herbert_gravesite_uuid,
    helen_gravesite.id AS helen_gravesite_uuid,
    herbert_gravesite.headstone_uuid
  FROM herbert_gravesite CROSS JOIN helen_gravesite
),
updated_herbert_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.herbert_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0345', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'herbert george brant'
    AND EXISTS (
...
updated_helen_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.helen_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0345-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'helen s brant'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, herbert_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_herbert_burial.id
  FROM marker_context CROSS JOIN updated_herbert_burial
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_helen_burial.id
  FROM marker_context CROSS JOIN updated_helen_burial
  ON CONFLICT (headstone_uuid, burial_uuid) DO UPDATE SET
    deleted_at = NULL, deleted_by = NULL, delete_reason = NULL
)
UPDATE headstones
SET gravesite_uuid = marker_context.herbert_gravesite_uuid, updated_at = now()
FROM marker_context
