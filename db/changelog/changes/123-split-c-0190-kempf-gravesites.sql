...
--changeset cemeterymapping:123-split-c-0190-kempf-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0190'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id IN ('TLC-GPS-0190', 'TLC-GPS-0190-02')
    AND upper(COALESCE(gravesites.section_id, '')) = 'C'
  ORDER BY gravesites.gravesite_id = 'TLC-GPS-0190' DESC
  LIMIT 1
),
...
peter_gravesite AS (
  ...
  SET
    name = 'Peter Kempf',
    grave_id = '0190B',
    gravesite_id = 'TLC-GPS-0190-02',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0190 using fixed marker TLC-HS-0190 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'Peter Kempf assigned to the southern gravesite when splitting shared Kempf marker on 2026-06-20.'
    ),
    updated_at = now()
...
anna_gravesite AS (
  ...
  SELECT
    ...
    status_type_id,
    'operational',
    'Split from TLC-GPS-0190 using fixed marker TLC-HS-0190 as shared north/south boundary.',
    'estimated',
    'Anna Kempf assigned to the northern gravesite when splitting shared Kempf marker on 2026-06-20.',
    now()
...
updated_peter_burials AS (
  ...
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'peter kempf'
...
--rollback UPDATE headstones SET gravesite_uuid = (SELECT id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0190-02') WHERE headstone_id = 'TLC-HS-0190';
