...
--changeset cemeterymapping:260-split-c-0274-hague-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0274'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0274'
      ...
      ST_SetSRID(
        ST_MakePolygon(
          ST_MakeLine(ARRAY[
            ...
            ST_Project(
              ST_Project(headstone_point::geography, 4 * 0.3048, 0),
              10 * 0.3048,
              pi() / 2
            )::geometry,
      ...
      ST_SetSRID(
        ST_MakePolygon(
          ST_MakeLine(ARRAY[
            ...
            ST_Project(
              ST_Project(headstone_point::geography, 4 * 0.3048, pi()),
              10 * 0.3048,
              pi() / 2
            )::geometry,
            ST_Project(headstone_point::geography, 10 * 0.3048, pi() / 2)::geometry,
...
edward_gravesite AS (
  UPDATE gravesites
  SET
    name = 'Edward George Hague',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0274 using fixed marker TLC-HS-0274 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'Edward George Hague retained in original gravesite C-0274 and moved south when splitting the shared Hague marker on 2026-07-25.'
    ),
    updated_at = now()
...
amelia_gravesite AS (
  ...
  SELECT
    ...
    status_type_id,
    'operational',
    'Split from TLC-GPS-0274 using fixed marker TLC-HS-0274 as shared north/south boundary.',
    'estimated',
    'Amelia Hague assigned to new northern gravesite C-0274A when splitting the shared Hague marker on 2026-07-25.',
    now()
...
hague_burials AS (
  ...
  WHERE burials.deleted_at IS NULL
    ...
    AND lower(split_part(trim(COALESCE(burials.first_name, '')), ' ', 1)) IN ('edward', 'amelia')
),
updated_edward_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.edward_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0274',
    updated_at = now()
  FROM marker_context, hague_burials
  WHERE burials.id = hague_burials.id
    AND hague_burials.normalized_given_name = 'edward'
...
updated_amelia_burials AS (
  ...
  SET
    ...
    gravesite_id = 'TLC-GPS-0274-01',
    updated_at = now()
  FROM marker_context, hague_burials
  WHERE burials.id = hague_burials.id
    AND hague_burials.normalized_given_name = 'amelia'
  RETURNING burials.id
),
...
marker_burial_links AS (
  ...
  FROM marker_context
  CROSS JOIN updated_edward_burials
  UNION ALL
