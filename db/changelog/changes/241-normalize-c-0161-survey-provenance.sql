...
--changeset cemeterymapping:241-normalize-c-0161-survey-provenance splitStatements:false
WITH marker AS (
  UPDATE headstones
  SET
    source_properties = COALESCE(source_properties, '{}'::jsonb) || jsonb_build_object(
      ...
      jsonb_build_object(
        ...
        'burialIdentitySource', 'headstone spreadsheet row 161',
    ...
    reviewed_by = 'migration 241 normalized survey provenance',
    reviewed_at = now(),
    updated_at = now()
  WHERE headstone_id = 'TLC-HS-0161'
...
SET
  notes = concat_ws(
    ...
    'Not listed in the North Hills Genealogists book. Burial and marker identity verified against surveyed marker TLC-HS-0161.'
  ),
  data_confidence = 'high',
  review_status = 'reviewed',
  review_notes = concat_ws(
    ...
    'Burial identity verified against surveyed marker TLC-HS-0161; no NHG entry exists for this marker.'
  ),
  source_conflict = false,
  reviewed_by = 'migration 241 normalized survey provenance',
  reviewed_at = now(),
