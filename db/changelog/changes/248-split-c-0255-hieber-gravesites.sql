...
--changeset cemeterymapping:248-split-c-0255-hieber-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0255'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0255'
...
george_gravesite AS (
  ...
  SET
    ...
    geometry_source = 'Split from TLC-GPS-0255 using fixed marker TLC-HS-0255 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'George Hieber retained in original gravesite C-0255 and moved south when splitting shared Hieber marker on 2026-07-23.'
    ),
    updated_at = now()
...
olive_gravesite AS (
  ...
  SELECT
    ...
    status_type_id,
    'operational',
    'Split from TLC-GPS-0255 using fixed marker TLC-HS-0255 as shared north/south boundary.',
    'estimated',
    'Olive Hieber assigned to the new northern gravesite C-0255A when splitting shared Hieber marker on 2026-07-23.',
    now()
...
marker_context AS (
  SELECT
    ...
    olive_gravesite.id AS olive_gravesite_uuid,
    george_gravesite.headstone_uuid
  FROM george_gravesite
  CROSS JOIN olive_gravesite
),
hieber_burials AS (
  SELECT
    burials.id,
  ...
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.last_name, '')) = 'hieber'
    AND lower(COALESCE(burials.first_name, '')) IN ('george', 'olive')
),
updated_george_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.george_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0255',
    updated_at = now()
  FROM marker_context, hieber_burials
  WHERE burials.id = hieber_burials.id
    AND (
      hieber_burials.normalized_full_name = 'george hieber'
      OR (hieber_burials.normalized_first_name = 'george' AND hieber_burials.normalized_last_name = 'hieber')
    )
  RETURNING burials.id
),
updated_olive_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.olive_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0255-01',
    updated_at = now()
  FROM marker_context, hieber_burials
  WHERE burials.id = hieber_burials.id
    AND (
      hieber_burials.normalized_full_name = 'olive hieber'
      OR (hieber_burials.normalized_first_name = 'olive' AND hieber_burials.normalized_last_name = 'hieber')
    )
  RETURNING burials.id
),
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, olive_gravesite_uuid, 'spans', now()
  FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_olive_burials.id
  FROM marker_context
  CROSS JOIN updated_olive_burials
