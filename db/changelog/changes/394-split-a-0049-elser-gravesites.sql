...
--changeset cemeterymapping:394-split-a-0049-elser-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0049' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0049' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0049 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'william g elser'
      AND headstones.headstone_id = 'TLC-HS-0049'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'eleanor h elser'
      AND headstones.headstone_id = 'TLC-HS-0049'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active William G Elser burial and one linked active Eleanor H Elser burial must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR NOT EXISTS (
    ...
    WHERE target.cemetery_id = (SELECT cemetery_id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0049' AND deleted_at IS NULL)
      AND (target.gravesite_id = 'TLC-GPS-0049-01' OR (target.section_id = 'A' AND target.grave_id = '0049A'))
  ),
  'new gravesite TLC-GPS-0049-01 / A-0049A must be unused'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR EXISTS (
    ...
    WHERE g.gravesite_id = 'TLC-GPS-0049' AND g.section_id = 'A' AND g.deleted_at IS NULL
      AND h.headstone_id = 'TLC-HS-0049' AND h.deleted_at IS NULL
      AND (SELECT count(*) FROM burials b JOIN headstone_burials hb ON hb.burial_uuid = b.id
        WHERE hb.headstone_uuid = h.id AND hb.deleted_at IS NULL AND b.deleted_at IS NULL
          AND b.gravesite_uuid = g.id AND b.full_name IN ('William G Elser', 'Eleanor H Elser')) = 2
  ),
  'both Elser burials and the marker must belong to active Section A gravesite TLC-GPS-0049'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0049' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0049'
...
william_gravesite AS (
  UPDATE gravesites
  SET name = 'William G Elser', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0049 using fixed marker TLC-HS-0049 as the boundary between A-0049 and A-0049A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'William G Elser retained in original gravesite A-0049 and moved south; Eleanor H Elser was assigned the new grave north of the fixed shared marker on 2026-09-15. Estimated overlaps with neighboring gravesites require field review; see ADR 0057.'
    ), updated_at = now()
...
eleanor_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Eleanor H Elser', facility_id,
    section_id, block_id, lot_id, '0049A', 'TLC-GPS-0049-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0049 using fixed marker TLC-HS-0049 as the boundary between A-0049 and A-0049A.',
    'estimated', 'Eleanor H Elser assigned to new gravesite A-0049A immediately north of A-0049 on 2026-09-15. Estimated overlaps with neighboring gravesites require field review; see ADR 0057.', now()
...
marker_context AS (
  SELECT william_gravesite.id AS william_gravesite_uuid,
    eleanor_gravesite.id AS eleanor_gravesite_uuid, william_gravesite.headstone_uuid
  FROM william_gravesite CROSS JOIN eleanor_gravesite
),
updated_william_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.william_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0049', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'william g elser'
    AND EXISTS (
...
updated_eleanor_burial AS (
  ...
  SET gravesite_uuid = marker_context.eleanor_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0049-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'eleanor h elser'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
