...
--changeset cemeterymapping:359-split-c-0403-bohn-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0403' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0403' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0403 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'george j bohn'
      AND headstones.headstone_id = 'TLC-HS-0403'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'emma e bohn'
      AND headstones.headstone_id = 'TLC-HS-0403'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active George J Bohn burial and one linked active Emma E Bohn burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0403' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0403'
...
george_gravesite AS (
  UPDATE gravesites
  SET name = 'George J Bohn', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0403 using fixed marker TLC-HS-0403 as the boundary between C-0403 and C-0403A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'George J Bohn retained in original gravesite C-0403 and moved south; Emma E Bohn was assigned the new grave north of the fixed shared marker on 2026-09-01.'
    ), updated_at = now()
...
emma_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Emma E Bohn', facility_id,
    section_id, block_id, lot_id, '0403A', 'TLC-GPS-0403-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0403 using fixed marker TLC-HS-0403 as the boundary between C-0403 and C-0403A.',
    'estimated', 'Emma E Bohn assigned to new gravesite C-0403A immediately north of C-0403 on 2026-09-01.', now()
...
marker_context AS (
  SELECT george_gravesite.id AS george_gravesite_uuid,
    emma_gravesite.id AS emma_gravesite_uuid, george_gravesite.headstone_uuid
  FROM george_gravesite CROSS JOIN emma_gravesite
),
updated_george_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.george_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0403', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'george j bohn'
    AND EXISTS (
...
updated_emma_burial AS (
  ...
  SET gravesite_uuid = marker_context.emma_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0403-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'emma e bohn'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
