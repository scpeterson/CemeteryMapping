...
--changeset cemeterymapping:234-split-c-0240-murray-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0240'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0240'
...
wesley_gravesite AS (
  UPDATE gravesites
  SET
    name = 'C Wesley Murray',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0240 using fixed marker TLC-HS-0240 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'C Wesley Murray retained in original gravesite C-0240 and moved south when splitting shared Murray marker on 2026-07-21.'
    ),
    updated_at = now()
...
helen_gravesite AS (
  ...
  SELECT
    ...
    status_type_id,
    'operational',
    'Split from TLC-GPS-0240 using fixed marker TLC-HS-0240 as shared north/south boundary.',
    'estimated',
    'Helen I Murray assigned to the new northern gravesite C-0240A when splitting shared Murray marker on 2026-07-21.',
    now()
  FROM wesley_gravesite
...
marker_context AS (
  SELECT
    wesley_gravesite.id AS wesley_gravesite_uuid,
    helen_gravesite.id AS helen_gravesite_uuid,
    wesley_gravesite.headstone_uuid
  FROM wesley_gravesite
...
murray_burials AS (
  ...
  WHERE burials.deleted_at IS NULL
    AND (
      lower(COALESCE(burials.full_name, '')) IN ('c. wesley murray', 'helen i murray')
      OR (
        lower(COALESCE(burials.last_name, '')) = 'murray'
        AND lower(COALESCE(burials.first_name, '')) IN ('c. wesley', 'helen i')
      )
    )
),
updated_wesley_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.wesley_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0240',
    updated_at = now()
  FROM marker_context, murray_burials
  WHERE burials.id = murray_burials.id
    AND (
      murray_burials.normalized_full_name = 'c. wesley murray'
      OR (murray_burials.normalized_first_name = 'c. wesley' AND murray_burials.normalized_last_name = 'murray')
    )
...
updated_helen_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.helen_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0240-01',
    updated_at = now()
  FROM marker_context, murray_burials
  WHERE burials.id = murray_burials.id
    AND (
      murray_burials.normalized_full_name = 'helen i murray'
      OR (murray_burials.normalized_first_name = 'helen i' AND murray_burials.normalized_last_name = 'murray')
    )
  RETURNING burials.id
),
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, helen_gravesite_uuid, 'spans', now()
  FROM marker_context
  UNION ALL
  SELECT headstone_uuid, wesley_gravesite_uuid, 'spans', now()
  FROM marker_context
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_wesley_burials.id
  FROM marker_context
  CROSS JOIN updated_wesley_burials
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_helen_burials.id
  FROM marker_context
  CROSS JOIN updated_helen_burials
