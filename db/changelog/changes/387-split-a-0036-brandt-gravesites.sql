...
--changeset cemeterymapping:387-split-a-0036-brandt-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0036' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0036' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0036 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'peter brandt'
      AND headstones.headstone_id = 'TLC-HS-0036'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'margaret brandt'
      AND headstones.headstone_id = 'TLC-HS-0036'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Peter Brandt burial and one linked active Margaret Brandt burial must exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites target
    WHERE target.gravesite_id = 'TLC-GPS-0036-01'
      AND (target.name IS DISTINCT FROM 'Margaret Brandt'
        OR target.cemetery_id IS DISTINCT FROM (
          SELECT cemetery_id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0036' AND deleted_at IS NULL
        ))
  ),
  'new gravesite TLC-GPS-0036-01 must be unused or already belong to Margaret Brandt in the source cemetery'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0036' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0036'
...
peter_gravesite AS (
  UPDATE gravesites
  SET name = 'Peter Brandt', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0036 using fixed marker TLC-HS-0036 as the boundary between A-0036 and A-0036A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Peter Brandt retained in original gravesite A-0036 and moved south; Margaret Brandt was assigned the new grave north of the fixed shared marker on 2026-09-10.'
    ), updated_at = now()
...
margaret_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Margaret Brandt', facility_id,
    section_id, block_id, lot_id, '0036A', 'TLC-GPS-0036-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0036 using fixed marker TLC-HS-0036 as the boundary between A-0036 and A-0036A.',
    'estimated', 'Margaret Brandt assigned to new gravesite A-0036A immediately north of A-0036 on 2026-09-10.', now()
...
marker_context AS (
  SELECT peter_gravesite.id AS peter_gravesite_uuid,
    margaret_gravesite.id AS margaret_gravesite_uuid, peter_gravesite.headstone_uuid
  FROM peter_gravesite CROSS JOIN margaret_gravesite
),
updated_peter_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.peter_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0036', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'peter brandt'
    AND EXISTS (
...
updated_margaret_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.margaret_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0036-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'margaret brandt'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, peter_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
  SELECT headstone_uuid, margaret_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_peter_burial.id
  FROM marker_context CROSS JOIN updated_peter_burial
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_margaret_burial.id
  FROM marker_context CROSS JOIN updated_margaret_burial
  ON CONFLICT (headstone_uuid, burial_uuid) DO UPDATE SET
    deleted_at = NULL, deleted_by = NULL, delete_reason = NULL
)
UPDATE headstones
SET gravesite_uuid = marker_context.peter_gravesite_uuid, updated_at = now()
FROM marker_context
