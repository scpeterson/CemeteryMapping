...
--changeset cemeterymapping:328-split-c-0342-brant-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id IN ('TLC-GPS-0341', 'TLC-GPS-0343') AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id = 'TLC-GPS-0342'
      AND upper(COALESCE(section_id, '')) = 'C'
      AND deleted_at IS NULL
  ),
  'active Section C gravesite TLC-GPS-0342 must exist when its neighboring operational gravesites exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0342' AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM headstones
    WHERE headstone_id = 'TLC-HS-0342' AND geometry IS NOT NULL AND deleted_at IS NULL
  ),
  'active marker TLC-HS-0342 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(full_name, '')) = 'edward a brant' AND deleted_at IS NULL
  ) = 1
  AND (
    SELECT count(*) FROM burials
    WHERE lower(COALESCE(full_name, '')) = 'sophia m brant' AND deleted_at IS NULL
  ) = 1,
  'exactly one active Edward A Brant burial and one active Sophia M Brant burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0342'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0342'
...
edward_gravesite AS (
  ...
  SET
    ...
    geometry_source = 'Split from TLC-GPS-0342 using fixed marker TLC-HS-0342 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Edward A Brant retained in original gravesite C-0342 and moved south when splitting the shared Brant marker on 2026-08-24.'
    ),
    updated_at = now()
...
sophia_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Sophia M Brant', facility_id,
    section_id, block_id, lot_id, '0342A', 'TLC-GPS-0342-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0342 using fixed marker TLC-HS-0342 as shared north/south boundary.',
    'estimated',
    'Sophia M Brant assigned to the new northern gravesite C-0342A when splitting the shared Brant marker on 2026-08-24.',
    now()
...
marker_context AS (
  SELECT
    ...
    sophia_gravesite.id AS sophia_gravesite_uuid,
    edward_gravesite.headstone_uuid
  FROM edward_gravesite CROSS JOIN sophia_gravesite
),
updated_edward_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.edward_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0342', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'edward a brant'
    AND EXISTS (
...
updated_sophia_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.sophia_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0342-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'sophia m brant'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id
...
marker_gravesite_links AS (
  ...
  UNION ALL
  SELECT headstone_uuid, sophia_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_sophia_burial.id
  FROM marker_context CROSS JOIN updated_sophia_burial
