...
--changeset cemeterymapping:356-split-c-0419-magee-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0419' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0419' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0419 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'ralph c magee jr.'
      AND headstones.headstone_id = 'TLC-HS-0419'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'jeannette b magee'
      AND headstones.headstone_id = 'TLC-HS-0419'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Ralph C Magee Jr. burial and one linked active Jeannette B Magee burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0419' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0419'
...
ralph_gravesite AS (
  UPDATE gravesites
  SET name = 'Ralph C Magee Jr.', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0419 using fixed marker TLC-HS-0419 as the boundary between C-0419 and C-0419A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Ralph C Magee Jr. retained in original gravesite C-0419 and moved south; Jeannette B Magee was assigned the new grave north of the fixed shared marker on 2026-09-01.'
    ),
    updated_at = now()
...
jeannette_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Jeannette B Magee', facility_id,
    section_id, block_id, lot_id, '0419A', 'TLC-GPS-0419-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0419 using fixed marker TLC-HS-0419 as the boundary between C-0419 and C-0419A.',
    'estimated',
    'Jeannette B Magee assigned to new gravesite C-0419A immediately north of C-0419 on 2026-09-01.',
    now()
...
marker_context AS (
  SELECT ralph_gravesite.id AS ralph_gravesite_uuid,
    jeannette_gravesite.id AS jeannette_gravesite_uuid,
    ralph_gravesite.headstone_uuid
  FROM ralph_gravesite CROSS JOIN jeannette_gravesite
),
updated_ralph_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.ralph_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0419', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'ralph c magee jr.'
    AND EXISTS (
...
updated_jeannette_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.jeannette_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0419-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'jeannette b magee'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  UNION ALL
  SELECT headstone_uuid, jeannette_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_jeannette_burial.id
  FROM marker_context CROSS JOIN updated_jeannette_burial
