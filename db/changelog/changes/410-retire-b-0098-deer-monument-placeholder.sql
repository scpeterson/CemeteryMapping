...
--changeset cemeterymapping:410-retire-b-0098-deer-monument-placeholder splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0098' AND deleted_at IS NULL)
  OR EXISTS (
    SELECT 1
    FROM headstones
    JOIN marker_scope_types ON marker_scope_types.id = headstones.marker_scope_type_id
    WHERE headstones.headstone_id = 'TLC-HS-0098'
      AND marker_scope_types.code = 'monolith'
      AND headstones.deleted_at IS NULL
  ),
  'active TLC-HS-0098 must be a monolith before retiring placeholder B-0098'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE gravesite_id = 'TLC-GPS-0098' AND deleted_at IS NULL
  ) = 1
  AND EXISTS (
    SELECT 1 FROM burials
    WHERE gravesite_id = 'TLC-GPS-0098'
      AND lower(trim(COALESCE(full_name, ''))) = 'deer family headstone'
      AND deleted_at IS NULL
  ),
  'B-0098 must contain only the generated Deer Family Headstone pseudo-burial'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR NOT EXISTS (
    ...
    WHERE placeholder.gravesite_id = 'TLC-GPS-0098'
  ...
  'B-0098 must not have ownership, maintenance, feature, or evidence dependencies before retirement'
);
    ...
    WHERE placeholder.gravesite_id = 'TLC-GPS-0098'
      ...
      AND NOT EXISTS (
        ...
        WHERE marker_media.media_asset_id = grave_media.media_asset_id
          AND marker.headstone_id = 'TLC-HS-0098'
          AND marker_media.deleted_at IS NULL
          AND marker_media.status = 'linked'
          AND marker.deleted_at IS NULL
      )
  ),
  'every active B-0098 photo must also be linked to TLC-HS-0098 before retirement'
);
...
WITH record_context AS (
  ...
  JOIN headstones marker
    ON marker.headstone_id = 'TLC-HS-0098' AND marker.deleted_at IS NULL
  JOIN burials pseudo_burial
    ON pseudo_burial.gravesite_uuid = placeholder.id
   AND lower(trim(COALESCE(pseudo_burial.full_name, ''))) = 'deer family headstone'
   AND pseudo_burial.deleted_at IS NULL
  WHERE placeholder.gravesite_id = 'TLC-GPS-0098' AND placeholder.deleted_at IS NULL
  LIMIT 1
),
retired_marker_burial_link AS (
  UPDATE headstone_burials
  SET deleted_at = now(), deleted_by = NULL,
    delete_reason = 'Generated Deer Family Headstone pseudo-burial retired; TLC-HS-0098 remains as a standalone family monument.'
  FROM record_context
...
retired_gravesite_media_links AS (
  ...
  SET deleted_at = now(), deleted_by = NULL,
    delete_reason = 'B-0098 placeholder retired; photos remain linked directly to TLC-HS-0098.',
    updated_at = now()
...
retired_marker_gravesite_link AS (
  ...
  SET deleted_at = now(), deleted_by = NULL,
    delete_reason = 'B-0098 represented the Deer family monument location, not a burial space.',
    updated_at = now()
  FROM record_context
...
updated_marker AS (
  UPDATE headstones
  SET gravesite_uuid = NULL,
    faces = COALESCE((
      SELECT jsonb_agg(jsonb_set(face, '{burialIds}', COALESCE((
        SELECT jsonb_agg(person_id) FROM jsonb_array_elements(face->'burialIds') person_id
        WHERE person_id <> to_jsonb(record_context.burial_uuid::text)
      ), '[]'::jsonb)) ORDER BY ordinal)
      FROM jsonb_array_elements(headstones.faces) WITH ORDINALITY AS f(face, ordinal)
    ), '[]'::jsonb),
    updated_at = now()
...
retired_pseudo_burial AS (
  UPDATE burials
  SET deleted_at = now(), deleted_by = NULL,
    delete_reason = 'Generated Deer Family Headstone pseudo-burial retired because the family monument is not a person or burial.',
    updated_at = now()
...
UPDATE gravesites
SET deleted_at = now(), deleted_by = NULL,
  delete_reason = 'B-0098 was an imported placeholder for family monument TLC-HS-0098 and is not a burial space.',
  updated_at = now()
