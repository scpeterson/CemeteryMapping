...
--changeset cemeterymapping:405-split-a-0078-dozer-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0078' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0078' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0078 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(concat_ws(' ', burials.first_name, burials.last_name)) = 'charles e dozer'
      AND headstones.headstone_id = 'TLC-HS-0078'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(concat_ws(' ', burials.first_name, burials.last_name)) = 'corinne e dozer'
      AND headstones.headstone_id = 'TLC-HS-0078'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Reverend Charles E Dozer burial and one linked active Corinne E Dozer burial must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR NOT EXISTS (
    ...
    WHERE target.cemetery_id = (SELECT cemetery_id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0078' AND deleted_at IS NULL)
      AND (target.gravesite_id = 'TLC-GPS-0078-01' OR (target.section_id = 'A' AND target.grave_id = '0078A'))
  ),
  'new gravesite TLC-GPS-0078-01 / A-0078A must be unused'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR EXISTS (
    ...
    WHERE g.gravesite_id = 'TLC-GPS-0078' AND g.section_id = 'A' AND g.deleted_at IS NULL
      AND h.headstone_id = 'TLC-HS-0078' AND h.deleted_at IS NULL
      AND (SELECT count(*) FROM burials b JOIN headstone_burials hb ON hb.burial_uuid = b.id
        WHERE hb.headstone_uuid = h.id AND hb.deleted_at IS NULL AND b.deleted_at IS NULL
          AND b.gravesite_uuid = g.id AND concat_ws(' ', b.first_name, b.last_name) IN ('Charles E Dozer', 'Corinne E Dozer')) = 2
  ),
  'both Dozer burials and the marker must belong to active Section A gravesite TLC-GPS-0078'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0078' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0078'
...
charles_gravesite AS (
  UPDATE gravesites
  SET name = 'Reverend Charles E Dozer', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0078 using fixed marker TLC-HS-0078 as the boundary between A-0078 and A-0078A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Reverend Charles E Dozer retained in original gravesite A-0078 and moved south; Corinne E Dozer was assigned the new grave north of the fixed shared marker on 2026-09-17. Estimated overlaps with neighboring gravesites, including A-0077A, require field review; see ADR 0066.'
    ), updated_at = now()
...
corinne_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Corinne E Dozer', facility_id,
    section_id, block_id, lot_id, '0078A', 'TLC-GPS-0078-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0078 using fixed marker TLC-HS-0078 as the boundary between A-0078 and A-0078A.',
    'estimated', 'Corinne E Dozer assigned to new gravesite A-0078A immediately north of A-0078 on 2026-09-17. Estimated overlaps with neighboring gravesites, including A-0077A, require field review; see ADR 0066.', now()
...
marker_context AS (
  SELECT charles_gravesite.id AS charles_gravesite_uuid,
    corinne_gravesite.id AS corinne_gravesite_uuid, charles_gravesite.headstone_uuid
  FROM charles_gravesite CROSS JOIN corinne_gravesite
),
updated_charles_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.charles_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0078', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(concat_ws(' ', burials.first_name, burials.last_name)) = 'charles e dozer'
    AND EXISTS (
...
updated_corinne_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.corinne_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0078-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(concat_ws(' ', burials.first_name, burials.last_name)) = 'corinne e dozer'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  UNION ALL
  SELECT headstone_uuid, corinne_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_corinne_burial.id
  FROM marker_context CROSS JOIN updated_corinne_burial
