...
--changeset cemeterymapping:343-split-c-0379-hieber-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id IN ('TLC-GPS-0378', 'TLC-GPS-0380') AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id = 'TLC-GPS-0379'
      AND upper(COALESCE(section_id, '')) = 'C'
      AND deleted_at IS NULL
  ),
  'active Section C gravesite TLC-GPS-0379 must exist when its neighboring operational gravesites exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0379' AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM headstones
    WHERE headstone_id = 'TLC-HS-0379' AND geometry IS NOT NULL AND deleted_at IS NULL
  ),
  'active marker TLC-HS-0379 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(full_name, '')) = 'albert d hieber' AND deleted_at IS NULL
  ) = 1
  AND (
    SELECT count(*) FROM burials
    WHERE lower(COALESCE(full_name, '')) = 'ida c hieber' AND deleted_at IS NULL
  ) = 1,
  'exactly one active Albert D Hieber burial and one active Ida C Hieber burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0379'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0379'
...
albert_gravesite AS (
  UPDATE gravesites
  SET
    name = 'Albert D Hieber',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0379 using fixed marker TLC-HS-0379 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Albert D Hieber retained in original gravesite C-0379 and moved south when splitting the shared Hieber marker on 2026-08-27.'
    ),
    updated_at = now()
...
ida_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Ida C Hieber', facility_id,
    section_id, block_id, lot_id, '0379A', 'TLC-GPS-0379-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0379 using fixed marker TLC-HS-0379 as shared north/south boundary.',
    'estimated',
    'Ida C Hieber assigned to new northern gravesite C-0379A when splitting the shared Hieber marker on 2026-08-27. No death date was recorded because none is known.',
    now()
...
marker_context AS (
  SELECT
    ...
    ida_gravesite.id AS ida_gravesite_uuid,
    albert_gravesite.headstone_uuid
  FROM albert_gravesite CROSS JOIN ida_gravesite
),
updated_albert_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.albert_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0379', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'albert d hieber'
    AND EXISTS (
...
updated_ida_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.ida_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0379-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'ida c hieber'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, albert_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
  SELECT headstone_uuid, ida_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_albert_burial.id
  FROM marker_context CROSS JOIN updated_albert_burial
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_ida_burial.id
  FROM marker_context CROSS JOIN updated_ida_burial
  ON CONFLICT (headstone_uuid, burial_uuid) DO UPDATE SET
    deleted_at = NULL, deleted_by = NULL, delete_reason = NULL
)
UPDATE headstones
SET gravesite_uuid = marker_context.albert_gravesite_uuid, updated_at = now()
FROM marker_context
