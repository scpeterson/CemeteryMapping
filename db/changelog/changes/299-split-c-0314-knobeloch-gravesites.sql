...
--changeset cemeterymapping:299-split-c-0314-knobeloch-gravesites splitStatements:false
WITH source_record AS (
  SELECT gravesites.*, headstones.id AS headstone_uuid,
    ST_SetSRID(headstones.geometry, 4326) AS headstone_point
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0314' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0314'
...
william_gravesite AS (
  UPDATE gravesites
  SET name = 'William G Knobeloch',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0314 using fixed marker TLC-HS-0314 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'William G Knobeloch retained in original gravesite C-0314 and moved south when splitting the shared Knobeloch marker on 2026-08-09.'
    ),
    updated_at = now()
...
emma_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Emma M Knobeloch', facility_id,
    section_id, block_id, lot_id, '0314A', 'TLC-GPS-0314-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0314 using fixed marker TLC-HS-0314 as shared north/south boundary.',
    'estimated',
    'Emma M Knobeloch assigned to the new northern gravesite C-0314A when splitting the shared Knobeloch marker on 2026-08-09.',
    now()
...
updated_william_burial AS (
  ...
  SET gravesite_uuid = marker_context.william_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0314', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'william g knobeloch'
  RETURNING burials.id
),
updated_emma_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.emma_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0314-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'emma m knobeloch'
  RETURNING burials.id
),
