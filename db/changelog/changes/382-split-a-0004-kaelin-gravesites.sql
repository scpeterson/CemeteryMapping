...
--changeset cemeterymapping:382-split-a-0004-kaelin-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0004' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0004' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0004 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'donald n kaelin'
      AND headstones.headstone_id = 'TLC-HS-0004'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'ellen m kaelin'
      AND headstones.headstone_id = 'TLC-HS-0004'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Donald N Kaelin burial and one linked active Ellen M Kaelin pre-need record must exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites target
    WHERE target.gravesite_id = 'TLC-GPS-0004-01'
      AND (target.name IS DISTINCT FROM 'Ellen M Kaelin'
        OR target.cemetery_id IS DISTINCT FROM (
          SELECT cemetery_id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0004' AND deleted_at IS NULL
        ))
  ),
  'new gravesite TLC-GPS-0004-01 must be unused or already belong to Ellen M Kaelin in the source cemetery'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0004' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0004'
...
donald_gravesite AS (
  UPDATE gravesites
  SET name = 'Donald N Kaelin', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0004 using fixed marker TLC-HS-0004 as the boundary between A-0004 and A-0004A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Donald N Kaelin retained in original gravesite A-0004 and moved south; Ellen M Kaelin was assigned the new pre-need grave north of the fixed shared marker on 2026-09-10.'
    ), updated_at = now()
...
ellen_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Ellen M Kaelin', facility_id,
    section_id, block_id, lot_id, '0004A', 'TLC-GPS-0004-01', cost, north_geometry,
    4.00, 10.00,
    (SELECT id FROM gravesite_status_types WHERE code = 'sold' AND is_active LIMIT 1),
    'operational',
    'Split from TLC-GPS-0004 using fixed marker TLC-HS-0004 as the boundary between A-0004 and A-0004A.',
    'estimated', 'Ellen M Kaelin assigned to new sold gravesite A-0004A immediately north of A-0004 as a pre-need inscription on 2026-09-10.', now()
...
marker_context AS (
  SELECT donald_gravesite.id AS donald_gravesite_uuid,
    ellen_gravesite.id AS ellen_gravesite_uuid, donald_gravesite.headstone_uuid
  FROM donald_gravesite CROSS JOIN ellen_gravesite
),
updated_donald_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.donald_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0004',
    burial_record_status_type_id = (SELECT id FROM burial_record_status_types WHERE code = 'interred'),
    updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'donald n kaelin'
    AND EXISTS (
...
updated_ellen_burial AS (
  ...
  SET gravesite_uuid = marker_context.ellen_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0004-01',
    burial_record_status_type_id = (SELECT id FROM burial_record_status_types WHERE code = 'pre_need_inscription'),
    burial_date = NULL, updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'ellen m kaelin'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, donald_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_donald_burial.id
  FROM marker_context CROSS JOIN updated_donald_burial
  UNION ALL
...
UPDATE headstones
SET gravesite_uuid = marker_context.donald_gravesite_uuid, updated_at = now()
FROM marker_context
