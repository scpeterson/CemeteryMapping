...
--changeset cemeterymapping:227-split-c-0224-happoldt-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0224'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0224'
...
edward_gravesite AS (
  UPDATE gravesites
  SET
    name = 'Edward G Happoldt',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0224 using fixed marker TLC-HS-0224 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'Edward G Happoldt retained in original gravesite C-0224 and moved south when splitting shared Happoldt marker on 2026-07-20.'
    ),
    updated_at = now()
...
margaret_gravesite AS (
  ...
  SELECT
    ...
    status_type_id,
    'operational',
    'Split from TLC-GPS-0224 using fixed marker TLC-HS-0224 as shared north/south boundary.',
    'estimated',
    'Margaret B Happoldt assigned to the new northern gravesite C-0224A when splitting shared Happoldt marker on 2026-07-20.',
    now()
...
happoldt_burials AS (
  ...
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) IN ('edward g happoldt', 'margaret b happoldt')
),
updated_edward_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.edward_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0224',
    updated_at = now()
  FROM marker_context, happoldt_burials
  WHERE burials.id = happoldt_burials.id
    AND happoldt_burials.normalized_full_name = 'edward g happoldt'
...
updated_margaret_burials AS (
  ...
  SET
    ...
    gravesite_id = 'TLC-GPS-0224-01',
    updated_at = now()
  FROM marker_context, happoldt_burials
  WHERE burials.id = happoldt_burials.id
    AND happoldt_burials.normalized_full_name = 'margaret b happoldt'
  RETURNING burials.id
),
