...
--changeset cemeterymapping:380-split-a-0032-trohaugh-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0032' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0032' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0032 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'frank t trohaugh'
      AND headstones.headstone_id = 'TLC-HS-0032'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'ruth r trohaugh'
      AND headstones.headstone_id = 'TLC-HS-0032'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Frank T Trohaugh burial and one linked active Ruth R Trohaugh burial must exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites target
    WHERE target.gravesite_id = 'TLC-GPS-0032-01'
      AND (target.name IS DISTINCT FROM 'Ruth R Trohaugh'
        OR target.cemetery_id IS DISTINCT FROM (
          SELECT cemetery_id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0032' AND deleted_at IS NULL
        ))
  ),
  'new gravesite TLC-GPS-0032-01 must be unused or already belong to Ruth R Trohaugh in the source cemetery'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0032' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0032'
...
frank_gravesite AS (
  UPDATE gravesites
  SET name = 'Frank T Trohaugh', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0032 using fixed marker TLC-HS-0032 as the boundary between A-0032 and A-0032A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Frank T Trohaugh retained in original gravesite A-0032 and moved south; Ruth R Trohaugh was assigned the new grave north of the fixed shared marker on 2026-09-10.'
    ), updated_at = now()
...
ruth_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Ruth R Trohaugh', facility_id,
    section_id, block_id, lot_id, '0032A', 'TLC-GPS-0032-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0032 using fixed marker TLC-HS-0032 as the boundary between A-0032 and A-0032A.',
    'estimated', 'Ruth R Trohaugh assigned to new gravesite A-0032A immediately north of A-0032 on 2026-09-10.', now()
  FROM frank_gravesite
...
marker_context AS (
  SELECT frank_gravesite.id AS frank_gravesite_uuid,
    ruth_gravesite.id AS ruth_gravesite_uuid, frank_gravesite.headstone_uuid
  FROM frank_gravesite CROSS JOIN ruth_gravesite
),
updated_frank_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.frank_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0032', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'frank t trohaugh'
    AND EXISTS (
...
updated_ruth_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.ruth_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0032-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'ruth r trohaugh'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, frank_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_frank_burial.id
  FROM marker_context CROSS JOIN updated_frank_burial
  UNION ALL
...
UPDATE headstones
SET gravesite_uuid = marker_context.frank_gravesite_uuid, updated_at = now()
FROM marker_context
