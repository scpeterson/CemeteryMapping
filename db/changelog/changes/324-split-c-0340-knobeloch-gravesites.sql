...
--changeset cemeterymapping:324-split-c-0340-knobeloch-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id IN ('TLC-GPS-0339', 'TLC-GPS-0341') AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id = 'TLC-GPS-0340'
      AND upper(COALESCE(section_id, '')) = 'C'
      AND deleted_at IS NULL
  ),
  'active Section C gravesite TLC-GPS-0340 must exist when its neighboring operational gravesites exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id = 'TLC-GPS-0340' AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM headstones
    WHERE headstone_id = 'TLC-HS-0340'
      AND geometry IS NOT NULL
      AND deleted_at IS NULL
  ),
  'active marker TLC-HS-0340 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(full_name, '')) = 'karl charles knobeloch' AND deleted_at IS NULL
  ) = 1
  AND (
    SELECT count(*) FROM burials
    WHERE lower(COALESCE(full_name, '')) = 'clara dorothea knobeloch' AND deleted_at IS NULL
  ) = 1,
  'exactly one active Karl Charles Knobeloch burial and one active Clara Dorothea Knobeloch burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0340'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0340'
...
karl_gravesite AS (
  UPDATE gravesites
  SET
    name = 'Karl Charles Knobeloch',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0340 using fixed marker TLC-HS-0340 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'Karl Charles Knobeloch retained in original gravesite C-0340 and moved south when splitting the shared Knobeloch marker on 2026-08-24.'
    ),
    updated_at = now()
...
clara_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Clara Dorothea Knobeloch', facility_id,
    section_id, block_id, lot_id, '0340A', 'TLC-GPS-0340-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0340 using fixed marker TLC-HS-0340 as shared north/south boundary.',
    'estimated',
    'Clara Dorothea Knobeloch assigned to the new northern gravesite C-0340A when splitting the shared Knobeloch marker on 2026-08-24.',
    now()
  FROM karl_gravesite
...
marker_context AS (
  SELECT
    karl_gravesite.id AS karl_gravesite_uuid,
    clara_gravesite.id AS clara_gravesite_uuid,
    karl_gravesite.headstone_uuid
  FROM karl_gravesite
  CROSS JOIN clara_gravesite
),
updated_karl_burial AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.karl_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0340',
    updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'karl charles knobeloch'
    AND EXISTS (
...
updated_clara_burial AS (
  ...
  SET
    ...
    gravesite_id = 'TLC-GPS-0340-01',
    updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'clara dorothea knobeloch'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, karl_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_karl_burial.id
  FROM marker_context CROSS JOIN updated_karl_burial
  UNION ALL
