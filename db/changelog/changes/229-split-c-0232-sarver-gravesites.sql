...
--changeset cemeterymapping:229-split-c-0232-sarver-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0232'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0232'
...
philip_gravesite AS (
  ...
  SET
    ...
    geometry_source = 'Split from TLC-GPS-0232 using fixed marker TLC-HS-0232 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'Philip Sarver retained in original gravesite C-0232 and moved south when splitting shared Sarver marker on 2026-07-21.'
    ),
    updated_at = now()
...
catherine_gravesite AS (
  ...
  SELECT
    ...
    status_type_id,
    'operational',
    'Split from TLC-GPS-0232 using fixed marker TLC-HS-0232 as shared north/south boundary.',
    'estimated',
    'Catherine Sarver assigned to the new northern gravesite C-0232A when splitting shared Sarver marker on 2026-07-21.',
    now()
...
marker_context AS (
  SELECT
    philip_gravesite.id AS philip_gravesite_uuid,
    catherine_gravesite.id AS catherine_gravesite_uuid,
    philip_gravesite.headstone_uuid
  FROM philip_gravesite
  CROSS JOIN catherine_gravesite
),
sarver_burials AS (
  SELECT
    burials.id,
  ...
  WHERE burials.deleted_at IS NULL
    AND (
      lower(COALESCE(burials.full_name, '')) IN ('philip sarver', 'catherine sarver')
      OR (
        lower(COALESCE(burials.last_name, '')) = 'sarver'
        AND lower(COALESCE(burials.first_name, '')) IN ('philip', 'catherine')
      )
    )
),
updated_philip_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.philip_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0232',
    updated_at = now()
  FROM marker_context, sarver_burials
  WHERE burials.id = sarver_burials.id
    AND (
      sarver_burials.normalized_full_name = 'philip sarver'
      OR (sarver_burials.normalized_first_name = 'philip' AND sarver_burials.normalized_last_name = 'sarver')
    )
  RETURNING burials.id
),
updated_catherine_burials AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.catherine_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0232-01',
    updated_at = now()
  FROM marker_context, sarver_burials
  WHERE burials.id = sarver_burials.id
    AND (
      sarver_burials.normalized_full_name = 'catherine sarver'
      OR (sarver_burials.normalized_first_name = 'catherine' AND sarver_burials.normalized_last_name = 'sarver')
    )
  RETURNING burials.id
),
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, catherine_gravesite_uuid, 'spans', now()
  FROM marker_context
  UNION ALL
  SELECT headstone_uuid, philip_gravesite_uuid, 'spans', now()
  FROM marker_context
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_philip_burials.id
  FROM marker_context
  CROSS JOIN updated_philip_burials
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_catherine_burials.id
  FROM marker_context
  CROSS JOIN updated_catherine_burials
