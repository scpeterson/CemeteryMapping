...
--changeset cemeterymapping:326-split-c-0341-ehrhardt-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id IN ('TLC-GPS-0340', 'TLC-GPS-0342') AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM gravesites
    WHERE gravesite_id = 'TLC-GPS-0341'
      AND upper(COALESCE(section_id, '')) = 'C'
      AND deleted_at IS NULL
  ),
  'active Section C gravesite TLC-GPS-0341 must exist when its neighboring operational gravesites exist'
);

SELECT assert_migration_prerequisite(
  NOT EXISTS (
    SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0341' AND deleted_at IS NULL
  )
  OR EXISTS (
    SELECT 1 FROM headstones
    WHERE headstone_id = 'TLC-HS-0341' AND geometry IS NOT NULL AND deleted_at IS NULL
  ),
  'active marker TLC-HS-0341 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(full_name, '')) = 'jv ehrhardt' AND deleted_at IS NULL
  ) = 1
  AND (
    SELECT count(*) FROM burials
    WHERE lower(COALESCE(full_name, '')) = 'anna m ehrhardt' AND deleted_at IS NULL
  ) = 1,
  'exactly one active JV Ehrhardt burial and one active Anna M Ehrhardt burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0341'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0341'
...
jv_gravesite AS (
  ...
  SET
    ...
    geometry_source = 'Split from TLC-GPS-0341 using fixed marker TLC-HS-0341 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'JV Ehrhardt retained in original gravesite C-0341 and moved south when splitting the shared Ehrhardt marker on 2026-08-24.'
    ),
    updated_at = now()
...
anna_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Anna M Ehrhardt', facility_id,
    section_id, block_id, lot_id, '0341A', 'TLC-GPS-0341-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0341 using fixed marker TLC-HS-0341 as shared north/south boundary.',
    'estimated',
    'Anna M Ehrhardt assigned to the new northern gravesite C-0341A when splitting the shared Ehrhardt marker on 2026-08-24.',
    now()
  FROM jv_gravesite
...
marker_context AS (
  SELECT
    jv_gravesite.id AS jv_gravesite_uuid,
    anna_gravesite.id AS anna_gravesite_uuid,
    jv_gravesite.headstone_uuid
  FROM jv_gravesite CROSS JOIN anna_gravesite
),
updated_jv_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.jv_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0341', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'jv ehrhardt'
    AND EXISTS (
...
updated_anna_burial AS (
  ...
  SET gravesite_uuid = marker_context.anna_gravesite_uuid,
      gravesite_id = 'TLC-GPS-0341-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'anna m ehrhardt'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, jv_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_jv_burial.id
  FROM marker_context CROSS JOIN updated_jv_burial
  UNION ALL
...
UPDATE headstones
SET gravesite_uuid = marker_context.jv_gravesite_uuid, updated_at = now()
FROM marker_context
