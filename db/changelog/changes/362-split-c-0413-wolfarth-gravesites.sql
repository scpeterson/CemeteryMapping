...
--changeset cemeterymapping:362-split-c-0413-wolfarth-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0413' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0413' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0413 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'theresa p wolfarth'
      AND headstones.headstone_id = 'TLC-HS-0413'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'raymond a wolfarth'
      AND headstones.headstone_id = 'TLC-HS-0413'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Theresa P Wolfarth burial and one linked active Raymond A Wolfarth burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0413' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0413'
...
theresa_gravesite AS (
  UPDATE gravesites
  SET name = 'Theresa P Wolfarth', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0413 using fixed marker TLC-HS-0413 as the boundary between C-0413 and C-0413A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Theresa P Wolfarth retained in original gravesite C-0413 and moved south; Raymond A Wolfarth was assigned the new grave north of the fixed shared marker on 2026-09-01.'
    ), updated_at = now()
...
raymond_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Raymond A Wolfarth', facility_id,
    section_id, block_id, lot_id, '0413A', 'TLC-GPS-0413-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0413 using fixed marker TLC-HS-0413 as the boundary between C-0413 and C-0413A.',
    'estimated', 'Raymond A Wolfarth assigned to new gravesite C-0413A immediately north of C-0413 on 2026-09-01.', now()
  FROM theresa_gravesite
...
marker_context AS (
  SELECT theresa_gravesite.id AS theresa_gravesite_uuid,
    raymond_gravesite.id AS raymond_gravesite_uuid, theresa_gravesite.headstone_uuid
  FROM theresa_gravesite CROSS JOIN raymond_gravesite
),
updated_theresa_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.theresa_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0413', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'theresa p wolfarth'
    AND EXISTS (
...
updated_raymond_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.raymond_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0413-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'raymond a wolfarth'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, theresa_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
  SELECT headstone_uuid, raymond_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_theresa_burial.id
  FROM marker_context CROSS JOIN updated_theresa_burial
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_raymond_burial.id
  FROM marker_context CROSS JOIN updated_raymond_burial
  ON CONFLICT (headstone_uuid, burial_uuid) DO UPDATE SET
    deleted_at = NULL, deleted_by = NULL, delete_reason = NULL
)
UPDATE headstones
SET gravesite_uuid = marker_context.theresa_gravesite_uuid, updated_at = now()
FROM marker_context
