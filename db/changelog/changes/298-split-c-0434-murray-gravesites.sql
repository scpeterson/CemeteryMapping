...
--changeset cemeterymapping:298-split-c-0434-murray-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0434'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0434'
...
herbert_gravesite AS (
  UPDATE gravesites
  SET
    name = 'Herbert W Murray',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0434 using fixed marker TLC-HS-0434 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'Herbert W Murray retained in original gravesite C-0434 and moved south when splitting the shared Murray marker on 2026-08-09.'
    ),
    updated_at = now()
...
martha_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Martha J Murray', facility_id, section_id, block_id, lot_id,
    '0434A', 'TLC-GPS-0434-01', cost, north_geometry, 4.00, 10.00, status_type_id,
    'operational',
    'Split from TLC-GPS-0434 using fixed marker TLC-HS-0434 as shared north/south boundary.',
    'estimated',
    'Martha J Murray assigned to the new northern gravesite C-0434A when splitting the shared Murray marker on 2026-08-09.',
    now()
...
updated_herbert_burial AS (
  UPDATE burials
  SET
    gravesite_uuid = marker_context.herbert_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0434',
    updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'herbert w murray'
...
updated_martha_burial AS (
  ...
  SET
    ...
    gravesite_id = 'TLC-GPS-0434-01',
    updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'martha j murray'
  RETURNING burials.id
),
...
marker_burial_links AS (
  ...
  FROM marker_context
  CROSS JOIN updated_herbert_burial
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_martha_burial.id
  FROM marker_context
  CROSS JOIN updated_martha_burial
