...
--changeset cemeterymapping:270-split-c-0294-kempf-gravesites splitStatements:false
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones
    ON headstones.headstone_id = 'TLC-HS-0294'
   AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0294'
...
john_gravesite AS (
  UPDATE gravesites
  SET
    name = 'John Peter Kempf',
    geometry = replacement_geometries.south_geometry,
    width_feet = 4.00,
    length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0294 using fixed marker TLC-HS-0294 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ',
      NULLIF(gravesites.geometry_notes, ''),
      'John Peter Kempf retained in original gravesite C-0294 and moved south when splitting the shared Kempf marker on 2026-07-28.'
    ),
    updated_at = now()
...
anna_gravesite AS (
  ...
  SELECT
    cemetery_id, section_uuid, block_uuid, lot_uuid, 'Anna Kempf', facility_id, section_id, block_id, lot_id,
    '0294A', 'TLC-GPS-0294-01', cost, north_geometry, 4.00, 10.00, status_type_id,
    'operational',
    'Split from TLC-GPS-0294 using fixed marker TLC-HS-0294 as shared north/south boundary.',
    'estimated',
    'Anna Kempf assigned to new northern gravesite C-0294A when splitting the shared Kempf marker on 2026-07-28.',
    now()
  FROM john_gravesite
...
marker_context AS (
  SELECT
    john_gravesite.id AS john_gravesite_uuid,
    anna_gravesite.id AS anna_gravesite_uuid,
    john_gravesite.headstone_uuid
  FROM john_gravesite
...
kempf_burials AS (
  ...
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.last_name, '')) = 'kempf'
    AND lower(split_part(trim(COALESCE(burials.first_name, '')), ' ', 1)) IN ('anna', 'john')
),
updated_john_burials AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.john_gravesite_uuid, gravesite_id = 'TLC-GPS-0294', updated_at = now()
  FROM marker_context, kempf_burials
  WHERE burials.id = kempf_burials.id
    AND kempf_burials.normalized_given_name = 'john'
  RETURNING burials.id
),
updated_anna_burials AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.anna_gravesite_uuid, gravesite_id = 'TLC-GPS-0294-01', updated_at = now()
  FROM marker_context, kempf_burials
  WHERE burials.id = kempf_burials.id
    AND kempf_burials.normalized_given_name = 'anna'
  RETURNING burials.id
),
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_john_burials.id
  FROM marker_context CROSS JOIN updated_john_burials
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_anna_burials.id
  FROM marker_context CROSS JOIN updated_anna_burials
