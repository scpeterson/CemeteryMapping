...
--changeset cemeterymapping:301-split-c-0432-muller-gravesites splitStatements:false
WITH source_record AS (
  SELECT gravesites.*, headstones.id AS headstone_uuid,
    ST_SetSRID(headstones.geometry, 4326) AS headstone_point
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0432' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0432'
...
mabel_gravesite AS (
  UPDATE gravesites
  SET name = 'Mabel F Muller', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00,
    geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0432 using fixed marker TLC-HS-0432 as shared north/south boundary.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Mabel F Muller retained in original gravesite C-0432 and moved south when splitting the shared Muller marker on 2026-08-10.'
    ),
    updated_at = now()
...
william_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'William N Muller', facility_id,
    section_id, block_id, lot_id, '0432A', 'TLC-GPS-0432-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0432 using fixed marker TLC-HS-0432 as shared north/south boundary.',
    'estimated',
    'William N Muller assigned to the new northern gravesite C-0432A when splitting the shared Muller marker on 2026-08-10.',
    now()
  FROM mabel_gravesite
...
marker_context AS (
  SELECT mabel_gravesite.id AS mabel_gravesite_uuid,
    william_gravesite.id AS william_gravesite_uuid, mabel_gravesite.headstone_uuid
  FROM mabel_gravesite CROSS JOIN william_gravesite
),
updated_mabel_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.mabel_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0432', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'mabel f muller'
  RETURNING burials.id
),
updated_william_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.william_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0432-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'william n muller'
  RETURNING burials.id
),
marker_gravesite_links AS (
  INSERT INTO headstone_gravesites (headstone_uuid, gravesite_uuid, relationship_type, updated_at)
  SELECT headstone_uuid, mabel_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL SELECT headstone_uuid, william_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_mabel_burial.id
  FROM marker_context CROSS JOIN updated_mabel_burial
  UNION ALL SELECT marker_context.headstone_uuid, updated_william_burial.id
  FROM marker_context CROSS JOIN updated_william_burial
  ON CONFLICT (headstone_uuid, burial_uuid) DO UPDATE SET
    deleted_at = NULL, deleted_by = NULL, delete_reason = NULL
)
UPDATE headstones
SET gravesite_uuid = marker_context.mabel_gravesite_uuid, updated_at = now()
FROM marker_context
