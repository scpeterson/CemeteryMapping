...
--changeset cemeterymapping:357-split-c-0401-kivlan-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0401' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0401' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0401 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'harold b kivlan jr.'
      AND headstones.headstone_id = 'TLC-HS-0401'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'grace c kivlan'
      AND headstones.headstone_id = 'TLC-HS-0401'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Harold B Kivlan Jr. burial and one linked active Grace C Kivlan burial must exist'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0401' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0401'
...
harold_gravesite AS (
  UPDATE gravesites
  SET name = 'Harold B Kivlan Jr.', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0401 using fixed marker TLC-HS-0401 as the boundary between C-0401 and C-0401A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Harold B Kivlan Jr. retained in original gravesite C-0401 and moved south; Grace C Kivlan was assigned the new grave north of the fixed shared marker on 2026-09-01.'
    ), updated_at = now()
...
grace_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Grace C Kivlan', facility_id,
    section_id, block_id, lot_id, '0401A', 'TLC-GPS-0401-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0401 using fixed marker TLC-HS-0401 as the boundary between C-0401 and C-0401A.',
    'estimated', 'Grace C Kivlan assigned to new gravesite C-0401A immediately north of C-0401 on 2026-09-01.', now()
  FROM harold_gravesite
...
marker_context AS (
  SELECT harold_gravesite.id AS harold_gravesite_uuid,
    grace_gravesite.id AS grace_gravesite_uuid, harold_gravesite.headstone_uuid
  FROM harold_gravesite CROSS JOIN grace_gravesite
),
updated_harold_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.harold_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0401', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'harold b kivlan jr.'
    AND EXISTS (
...
updated_grace_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.grace_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0401-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'grace c kivlan'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  SELECT headstone_uuid, harold_gravesite_uuid, 'spans', now() FROM marker_context
  UNION ALL
...
marker_burial_links AS (
  ...
  SELECT marker_context.headstone_uuid, updated_harold_burial.id
  FROM marker_context CROSS JOIN updated_harold_burial
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_grace_burial.id
  FROM marker_context CROSS JOIN updated_grace_burial
  ON CONFLICT (headstone_uuid, burial_uuid) DO UPDATE SET
    deleted_at = NULL, deleted_by = NULL, delete_reason = NULL
)
UPDATE headstones
SET gravesite_uuid = marker_context.harold_gravesite_uuid, updated_at = now()
FROM marker_context
