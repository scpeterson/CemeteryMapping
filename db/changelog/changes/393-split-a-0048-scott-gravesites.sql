...
--changeset cemeterymapping:393-split-a-0048-scott-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0048' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0048' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0048 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'charles d scott'
      AND headstones.headstone_id = 'TLC-HS-0048'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'malinda c scott'
      AND headstones.headstone_id = 'TLC-HS-0048'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Charles D Scott burial and one linked active Malinda C Scott burial must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR NOT EXISTS (
    ...
    WHERE target.cemetery_id = (SELECT cemetery_id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0048' AND deleted_at IS NULL)
      AND (target.gravesite_id = 'TLC-GPS-0048-01' OR (target.section_id = 'A' AND target.grave_id = '0048A'))
  ),
  'new gravesite TLC-GPS-0048-01 / A-0048A must be unused'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR EXISTS (
    ...
    WHERE g.gravesite_id = 'TLC-GPS-0048' AND g.section_id = 'A' AND g.deleted_at IS NULL
      AND h.headstone_id = 'TLC-HS-0048' AND h.deleted_at IS NULL
      AND (SELECT count(*) FROM burials b JOIN headstone_burials hb ON hb.burial_uuid = b.id
        WHERE hb.headstone_uuid = h.id AND hb.deleted_at IS NULL AND b.deleted_at IS NULL
          AND b.gravesite_uuid = g.id AND b.full_name IN ('Charles D Scott', 'Malinda C Scott')) = 2
  ),
  'both Scott burials and the marker must belong to active Section A gravesite TLC-GPS-0048'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0048' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0048'
...
charles_gravesite AS (
  UPDATE gravesites
  SET name = 'Charles D Scott', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0048 using fixed marker TLC-HS-0048 as the boundary between A-0048 and A-0048A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Charles D Scott retained in original gravesite A-0048 and moved south; Malinda C Scott was assigned the new grave north of the fixed shared marker on 2026-09-15. Estimated southern boundary overlaps with A-0047 and A-0061 require field review; see ADR 0056.'
    ), updated_at = now()
...
malinda_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Malinda C Scott', facility_id,
    section_id, block_id, lot_id, '0048A', 'TLC-GPS-0048-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0048 using fixed marker TLC-HS-0048 as the boundary between A-0048 and A-0048A.',
    'estimated', 'Malinda C Scott assigned to new gravesite A-0048A immediately north of A-0048 on 2026-09-15.', now()
...
marker_context AS (
  SELECT charles_gravesite.id AS charles_gravesite_uuid,
    malinda_gravesite.id AS malinda_gravesite_uuid, charles_gravesite.headstone_uuid
  FROM charles_gravesite CROSS JOIN malinda_gravesite
),
updated_charles_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.charles_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0048', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'charles d scott'
    AND EXISTS (
...
updated_malinda_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.malinda_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0048-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'malinda c scott'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  UNION ALL
  SELECT headstone_uuid, malinda_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_malinda_burial.id
  FROM marker_context CROSS JOIN updated_malinda_burial
