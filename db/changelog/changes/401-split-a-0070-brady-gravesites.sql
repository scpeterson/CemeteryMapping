...
--changeset cemeterymapping:401-split-a-0070-brady-gravesites splitStatements:false
SELECT assert_migration_prerequisite(
  NOT EXISTS (SELECT 1 FROM gravesites WHERE gravesite_id = 'TLC-GPS-0070' AND deleted_at IS NULL)
  OR EXISTS (SELECT 1 FROM headstones WHERE headstone_id = 'TLC-HS-0070' AND geometry IS NOT NULL AND deleted_at IS NULL),
  'active marker TLC-HS-0070 with geometry must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR (
    ...
    WHERE lower(COALESCE(burials.full_name, '')) = 'charles m brady'
      AND headstones.headstone_id = 'TLC-HS-0070'
  ...
  AND (
    ...
    JOIN headstone_burials ON headstone_burials.burial_uuid = burials.id
    JOIN headstones ON headstones.id = headstone_burials.headstone_uuid
    WHERE lower(COALESCE(burials.full_name, '')) = 'marion milford brady'
      AND headstones.headstone_id = 'TLC-HS-0070'
      AND burials.deleted_at IS NULL AND headstone_burials.deleted_at IS NULL AND headstones.deleted_at IS NULL
  ) = 1,
  'exactly one linked active Charles M Brady burial and one linked active Marion Milford Brady burial must exist'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR NOT EXISTS (
    ...
    WHERE target.cemetery_id = (SELECT cemetery_id FROM gravesites WHERE gravesite_id = 'TLC-GPS-0070' AND deleted_at IS NULL)
      AND (target.gravesite_id = 'TLC-GPS-0070-01' OR (target.section_id = 'A' AND target.grave_id = '0070A'))
  ),
  'new gravesite TLC-GPS-0070-01 / A-0070A must be unused'
);
...
SELECT assert_migration_prerequisite(
  ...
  OR EXISTS (
    ...
    WHERE g.gravesite_id = 'TLC-GPS-0070' AND g.section_id = 'A' AND g.deleted_at IS NULL
      AND h.headstone_id = 'TLC-HS-0070' AND h.deleted_at IS NULL
      AND (SELECT count(*) FROM burials b JOIN headstone_burials hb ON hb.burial_uuid = b.id
        WHERE hb.headstone_uuid = h.id AND hb.deleted_at IS NULL AND b.deleted_at IS NULL
          AND b.gravesite_uuid = g.id AND b.full_name IN ('Charles M Brady', 'Marion Milford Brady')) = 2
  ),
  'both Brady burials and the marker must belong to active Section A gravesite TLC-GPS-0070'
);
...
WITH source_record AS (
  ...
  FROM gravesites
  JOIN headstones ON headstones.headstone_id = 'TLC-HS-0070' AND headstones.deleted_at IS NULL
  WHERE gravesites.deleted_at IS NULL
    AND gravesites.gravesite_id = 'TLC-GPS-0070'
...
charles_gravesite AS (
  UPDATE gravesites
  SET name = 'Charles M Brady', geometry = replacement_geometries.south_geometry,
    width_feet = 4.00, length_feet = 10.00, geometry_type = 'operational',
    geometry_source = 'Split from TLC-GPS-0070 using fixed marker TLC-HS-0070 as the boundary between A-0070 and A-0070A.',
    geometry_confidence = 'estimated',
    geometry_notes = concat_ws(
      ' ', NULLIF(gravesites.geometry_notes, ''),
      'Charles M Brady retained in original gravesite A-0070 and moved south; Marion Milford Brady was assigned the new grave north of the fixed shared marker on 2026-09-16. Small estimated overlaps with neighboring gravesites require field review; see ADR 0063.'
    ), updated_at = now()
...
marion_gravesite AS (
  ...
  SELECT cemetery_id, section_uuid, block_uuid, lot_uuid, 'Marion Milford Brady', facility_id,
    section_id, block_id, lot_id, '0070A', 'TLC-GPS-0070-01', cost, north_geometry,
    4.00, 10.00, status_type_id, 'operational',
    'Split from TLC-GPS-0070 using fixed marker TLC-HS-0070 as the boundary between A-0070 and A-0070A.',
    'estimated', 'Marion Milford Brady assigned to new gravesite A-0070A immediately north of A-0070 on 2026-09-16. Small estimated overlaps with neighboring gravesites require field review; see ADR 0063.', now()
...
marker_context AS (
  SELECT charles_gravesite.id AS charles_gravesite_uuid,
    marion_gravesite.id AS marion_gravesite_uuid, charles_gravesite.headstone_uuid
  FROM charles_gravesite CROSS JOIN marion_gravesite
),
updated_charles_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.charles_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0070', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'charles m brady'
    AND EXISTS (
...
updated_marion_burial AS (
  UPDATE burials
  SET gravesite_uuid = marker_context.marion_gravesite_uuid,
    gravesite_id = 'TLC-GPS-0070-01', updated_at = now()
  FROM marker_context
  WHERE burials.deleted_at IS NULL
    AND lower(COALESCE(burials.full_name, '')) = 'marion milford brady'
    AND EXISTS (
      SELECT 1 FROM headstone_burials
      WHERE headstone_burials.headstone_uuid = marker_context.headstone_uuid
        AND headstone_burials.burial_uuid = burials.id AND headstone_burials.deleted_at IS NULL
...
marker_gravesite_links AS (
  ...
  UNION ALL
  SELECT headstone_uuid, marion_gravesite_uuid, 'spans', now() FROM marker_context
...
marker_burial_links AS (
  ...
  UNION ALL
  SELECT marker_context.headstone_uuid, updated_marion_burial.id
  FROM marker_context CROSS JOIN updated_marion_burial
