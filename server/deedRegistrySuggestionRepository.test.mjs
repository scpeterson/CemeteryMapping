  ...
  const pool = {
    async query(sql, values) {
      ...
      return { rows: [{
        ...
        updated_known_date: "1944", updated_deed_on_file: "No", updated_deed_register_on_file: "Yes", updated_remarks: "North side",
...
test("deed registry suggestions retain both labeled remarks when the tabs differ", async () => {
  const pool = { async query() { return { rows: [{
    updated_id: "updated-id", updated_row_number: 2, updated_owner_name: "Test Owner", updated_remarks: "Updated text",
    original_id: "original-id", original_row_number: 1, original_owner_name: "Test Owner", original_remarks: "Original text",
  }] }; } };
  const [result] = await findDeedRegistrySuggestions(pool, "cemetery-id", "Test Owner");
  assert.equal(result.notes, "Original 2017 tab - line 1\nUpdated 2022 tab - line 2\nOriginal 2017 remarks: Original text\nUpdated 2022 remarks: Updated text");
});
