...
| Known limitations | Trinity Lutheran Church Cemetery does not use blocks; its lots are scoped directly to sections, with `block_id` and `block_uuid` left null. Other cemeteries may use blocks because the hierarchy supports them as optional records. Lot dimensions vary: common Trinity Section C rows use 10-foot-by-20-foot lots, smaller rows use 10-foot-by-12-foot lots, and reviewed historic-map rows include approximately 16.36-foot-by-16-foot lots. Section geometry is required after the Trinity Section B/G refresh, so known sections should not be left with null geometry. |
...
| Current use | Staged review evidence for gravesite-to-lot or passageway interpretation, plus reviewed anchor measurements for explicitly documented production lot geometry. |
...
| C-0217 shared-marker split | `C-0217` retains Julius L Hague; new northern `C-0217A` contains Katherine M Hague. Observed marker `TLC-HS-0217` remains fixed and spans both gravesites. |
| C-0220 shared-marker split | `C-0220` retains Jacob Hague; new northern `C-0220A` contains Margaret Caroline Hague. Observed marker `TLC-HS-0220` remains fixed and spans both gravesites. |
| C-0224 shared-marker split | `C-0224` retains Edward G Happoldt; new northern `C-0224A` contains Margaret B Happoldt. Observed marker `TLC-HS-0224` remains fixed and spans both gravesites. |
| C-0228 shared-marker split | `C-0228` retains Emile J Schug; new northern `C-0228A` contains Hazel M Schug. Observed marker `TLC-HS-0228` remains fixed and spans both gravesites. |
| C-0232 shared-marker split | `C-0232` retains Philip Sarver; new northern `C-0232A` contains Catherine Sarver. Observed marker `TLC-HS-0232` remains fixed and spans both gravesites. |
| C-0234 shared-marker split | `C-0234` retains Laverne J Hague; new northern `C-0234A` contains Clara H Hague. Observed marker `TLC-HS-0234` remains fixed and spans both gravesites. |
| C-0235 shared-marker split | `C-0235` retains Kenneth B Soergel; new northern `C-0235A` is reserved for living pre-need marker record Clara Pearle Soergel. Observed marker `TLC-HS-0235` remains fixed and spans both gravesites. |
| C-0236 shared-marker split | `C-0236` retains Howard L Soergel; new northern `C-0236A` contains Elsie G Soergel. Observed marker `TLC-HS-0236` remains fixed and spans both gravesites. |
| C-0257 shared-marker split | `C-0257` retains James M Sarver; new northern `C-0257A` contains Margaret E Sarver. Observed marker `TLC-HS-0257` remains fixed and spans both gravesites. |
| C-0258 shared-marker split | `C-0258` retains Walter H Schuessler; new northern `C-0258A` contains Armella M Schuessler. Observed marker `TLC-HS-0258` remains fixed and spans both gravesites. |
| Later shared-marker splits | Migrations `341` through `364` apply the reviewed fixed-marker north/south pattern to the assignments recorded in [ADR 0042](adr/0042-split-reviewed-section-c-shared-marker-gravesites.md), including `C-0367` through `C-0421`. Migration `352` corrects `C-0392A` so it is immediately north of `C-0392`. |
| A-0017 monolith placeholder | Migration `365` retires imported placeholder gravesite `A-0017` and its generated pseudo-burial. Observed Burgess family monument `TLC-HS-0017` remains active as a standalone monolith, and its two photos remain marker evidence. See [ADR 0045](adr/0045-retire-a-0017-burgess-monolith-placeholder.md). |
...
| Northern Section C rows | Migrations `274` through `277` create lots `C-101` through `C-120` in four reviewed rows. The rows use 10-foot widths and either 12-foot or 20-foot depths; `C-105`, `C-115`, and `C-120` are `non_burial` and cannot contain gravesites or markers. |
| Non-standard Section C rows | Migrations `278` through `280` create four six-lot rows reconstructed from historic-map measurements. Each lot is approximately 16.36 feet wide by 16 feet deep. The first block is anchored between `C-101` and the eastern edge of `C-11`; the later block aligns west with `C-74A` and south with `C-111`. |
| Duplicate lot-number rule | Lot numbers are not globally unique. The same number can occur in different cemetery areas, such as `70 OC` and `70 NA`. When historic maps repeat a number within Section C, reviewed production lots retain the earlier identifier and append `A` to the new lot's name and `lot_id`, such as `C-73A`. Evidence must always be interpreted with section, area, and geometry context. |
...
| Review UI | Admin -> Readings; admins can link, reject, flag candidate gravesite/headstone matches, use `Next unresolved` within the current filters, select multiple readings, mark selected readings reviewed, or send selected readings to Admin -> Bulk for a shared source note |
