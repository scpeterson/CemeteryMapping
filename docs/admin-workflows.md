...
The Admin UI has a Deed Evidence tab for reviewing staged deed registry imports before any promotion into production lot, gravesite, owner, or ownership-event tables. Admins can select an import batch, filter by parser confidence, filter by staged evidence type, search owner/lot/section/remark text, and review parser notes alongside related `Investigated` worksheet notes. For `Original 2017` and `Updated 2022` owner rows, admins can populate `ModernSection` and correct the lot number, last known date, and remarks directly in the application; source spreadsheet values are preserved separately. The `Updated 2022` rows also expose the imported `Deed on File` and `Deed Register on File` values for review and assisted ownership entry. Last-known dates retain the precision recorded by the source, including year-only values such as `1944`. When an `Original 2017` staging batch exists, the tab compares the selected current batch, usually `Updated 2022`, to that original baseline and highlights added, changed, unchanged, and removed owner rows.

When recording ownership from a selected gravesite, the assisted-fill action can reuse the matched registry source, tab and line references, remarks, last-known date, deed-file flags, owner name, and available address. Review every suggested value before saving because imported lot and gravesite mappings remain evidence rather than automatic proof of the modern spatial relationship.
...
Burial military service details distinguish branch, rank or rating, war service, and decorations. Decorations such as the Purple Heart appear as additional labeled badges beside the general Veteran badge; they do not replace veteran status. Historical U.S. Navy inscriptions such as `FIREMAN 1/C` can use `Fireman First Class` (`F1c`), with no assumed modern pay grade. Preserve the exact wording in the marker inscription. Navy rating abbreviations such as `MM2` should use the corresponding rating-specific rank value when the inscription provides it. World War II U.S. Army records can use `Technician Fifth Grade` with abbreviation `T/5`. Historical U.S. Air Force records can use `Airman Second Class` (`A2C`, `E-3`), separately from the modern Airman First Class entry. Select Vietnam independently as war service when supported by the person's record; adding a rank does not assign war service. Military ranks are controlled migration data and are not currently created through the Admin Lookups tab.

Historical E-3 terminology is supported by the [Air Force Board for Correction of Military Records, BC-2019-01994](https://boards.law.af.mil/AF/BCMR/CY2019/BC-2019-01994%20BC201901994.pdf), describing a former airman second class released from active duty in 1965.
