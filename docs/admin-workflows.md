...
Burial military service details distinguish branch, rank or rating, war service, and decorations. Decorations such as the Purple Heart appear as additional labeled badges beside the general Veteran badge; they do not replace veteran status. Navy rating abbreviations such as `MM2` should use the corresponding rating-specific rank value when the inscription provides it. World War II U.S. Army records can use `Technician Fifth Grade` with abbreviation `T/5`. Military ranks are controlled migration data and are not currently created through the Admin Lookups tab.
