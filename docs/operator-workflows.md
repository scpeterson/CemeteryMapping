...
Gravesite status labels are not all manually assigned. `reserved` and `needs_review` are human workflow flags, while `occupied`, `available`, and `unknown` are derived from active burials and current deed or ownership-right records. Keep the deprecated `sold` lookup inactive; ownership is maintained through deed and ownership-right workflows instead.
