import { NorthHillsOcrCandidateMatch } from "../../../types/northHills";
import {
...
export function ReadingMatches({ savingEvidenceKey, saveNorthHillsEvidence, canUnlinkNorthHillsEvidence, unlinkNorthHillsEvidence, entry }: Pick<Props, "savingEvidenceKey" | "saveNorthHillsEvidence" | "canUnlinkNorthHillsEvidence" | "unlinkNorthHillsEvidence"> & { entry: Props["northHillsOcrReview"]["entries"][number] }) {
  const renderMatch = (match: NorthHillsOcrCandidateMatch) => {
  ...
  return (
    <section className="deed-investigation-links" aria-label="Possible existing burial matches">
      ...
      {entry.candidateMatches.map(renderMatch)}
      {entry.additionalCandidateMatches?.length ? (
        <details>
          <summary>Show more possible matches ({entry.additionalCandidateMatches.length})</summary>
          <p>These extra suggestions do not affect completion status. Link a gravesite, reject it, or mark it for a field check to add your decision to this reading.</p>
          {entry.additionalCandidateMatches.map(renderMatch)}
        </details>
      ) : null}
    </section>
  );
