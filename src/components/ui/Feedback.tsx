...
export function StatusBadge({ children, tone = "neutral", color }: {
  children: ReactNode;
  tone?: "neutral" | "success" | "warning";
  color?: string;
}) {
  return <span
    className={`ui-status-badge ui-status-badge--${tone}${color ? " ui-status-badge--colored" : ""}`}
    style={color ? { "--status-color": color } as CSSProperties : undefined}
  >{children}</span>;
}
